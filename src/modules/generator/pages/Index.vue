<template>
    <div class="generator-pages-list">
        <vued-filter>
            <ul class="left">
...
export default {
    ...
    computed: {
        ...mapGetters('generator/pages', [
            'filters',
            'list',
            'total'
        ]),
        name: filtersCommit('generator/pages', 'name')
    },
    watch: {
        filters: {
            handler(nv, ov) {
                this.$store.dispatch('generator/pages/find');
            },
            deep: true
        }
    },
    ...
    methods: {
        handleAddClick() {
            this.$router.push({ name: 'generator-pages-add' });
        },
        handleEditClick({ id }) {
            this.$router.push({ name: 'generator-pages-edit', params: { id } });
        },
        handleDelClick({ id }) {
            // this.$router.push({ name: 'generator-pages-update' });
        },
        handleViewClick({ id }) {
            this.$router.push({ name: 'generator-pages-view', params: { id } });
        },
        handleSizeChange(size) {
            const page = 1;
            this.$store.commit('generator/pages/FILTERS', { size, page });
            this.$router.push({
        ...
        handlePageChange(page) {
            this.$store.commit('generator/pages/FILTERS', { page });
            this.$router.push({
                name: 'generator-pages',
                params: {
                    page
